"! @testing BDEF:ZR_PSBTP_ATRAV
CLASS zcl_psbtp_rap100_unit_test DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC
  FOR TESTING
...
CLASS zcl_psbtp_rap100_unit_test IMPLEMENTATION.
  METHOD class_setup.
    " create the test doubles for the underlying CDS entities
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
                      i_for_entities = VALUE #(
                        ( i_for_entity = 'ZR_PSBTP_ATRAV' ) ) ).
  ...
  METHOD create_with_action.
    ...
    MODIFY ENTITIES OF zr_psbtp_atrav
     ENTITY Travel
    ...
    SELECT * FROM zr_psbtp_atrav INTO TABLE @DATA(lt_travel). "#EC CI_NOWHERE
